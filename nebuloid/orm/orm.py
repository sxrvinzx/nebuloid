...
class NebuloidORM:
    def __init__(self, manifest, tools, use_flask):
    ...
    def connect_db(self, cred):
        ...
        if self.use_flask:
            ...
            return create_engine(url, echo=cred.get("echo", False), pool_pre_ping=True)
        return create_async_engine(url, echo=True)  # echo=True shows SQL logs
    
    @asynccontextmanager
    async def session_scope(self):
        """Works as both async and sync context manager depending on mode."""
        if not self.use_flask:
            async with self.Session() as session:
                yield session
        else:
            # run sync context in a thread-safe way
            @contextmanager
            def _sync_scope():
                with self.Session() as session:
                    yield session

            # wrap the sync context manager into an async one
            loop = None
            try:
            ...
            if loop and loop.is_running():
                # we're already inside an async loop (Quart)
                cm = _sync_scope()
                with cm as session:
                    yield session
            else:
                # no async loop — just run directly
                with self.Session() as session:
                    yield session
    async def execute(self, query, data=None):
        try:
            async with self.session_scope() as session:
                result = await self.tools.maybe_await(session.execute(text(query), data or {}))
             # commit if not a SELECT
                if result.returns_rows:  # SELECT queries
                    rows = [dict(row._mapping) for row in result.fetchall()]
                    return {"status": "success", "data": rows}
                else:  # INSERT/UPDATE/DELETE
                    await self.tools.maybe_await(session.commit())
    ...
    async def get_key(self, session_id: str):
        async with self.session_scope() as session:
            result = await self.tools.maybe_await(session.execute(
        ...
        while True:
            try:
                async with self.session_scope() as db:
                    await self.tools.maybe_await(db.execute(text("CALL maintenance()")))
                    await self.tools.maybe_await(db.commit())
    ...
    async def add_user(self, username: str, password_hash: str, role: int = 0):
        async with self.session_scope() as session:
            ...
            await self.tools.maybe_await(session.commit())
            await self.tools.maybe_await(session.refresh(new_user))  # ensures new_user.id is available
            return new_user
      ...
      #Check auth_keys row by session_id.
       #If logged_in=1, then fetch role from users table.
        ...
        async with self.session_scope() as session:
            ...
            user_result = await self.tools.maybe_await(session.execute(
            ...
            role_result = await self.tools.maybe_await(session.execute(
